...
for i in range(test_len):
    ...
    for file in sorted(os.listdir(dir)):
        if file.endswith(".csv"):
            ...
            valid_smiles = [smiles for smiles in df["smiles"] if smiles is not np.nan and Chem.MolFromSmiles(str(smiles))is not None]
...
print(", Validity, Uniqueness, Diversity, Dockable, IEVCosSim≥0.7\n")
...
with open(f'./results/{protein}/metrics.csv', 'w') as f:
    f.write("model, validity, uniqueness, diversity, docking, ievcos\n")
    for model, japanese_model in zip(models, japanese_models):
        f.write(japanese_model + ", " + str(model_dict[model]['MEAN']['smiles_valid']) + ", " + str(model_dict[model]['MEAN']['uniqueness']) + ", " + str(model_dict[model]['MEAN']['diversity']) + ", " + str(model_dict[model]['MEAN']['iev_valid']) + ", " + str(model_dict[model]['MEAN']['cos_07']) + "\n" )
    f.close()
