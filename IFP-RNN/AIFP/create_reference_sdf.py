...
def mol2_pdbqt(mols, config, thread=0):
    ...
    for i, ligand in enumerate(mols):
        ...
        ligand.write('mol2',
                     ...
                     overwrite=True)
        os.system(
            f'conda run -n vina {Path(config["prepare_ligand4"]).parent/"prepare_ligand4.py"} -l {ligand.title}_{ligand.data["Pose_id"]}.mol2 -o {ligand.title}_{ligand.data["Pose_id"]}_out.pdbqt'
        )
...
def main(args):
    ...
    os.system(
        f'conda run -n vina {Path(config["prepare_ligand4"]).parent/"prepare_receptor4.py"} -r {args.protein} -o ./obj/protein.pdbqt'
    )
