...
from sklearn.preprocessing import StandardScaler  # For the descriptorsssssss
...
class DDC:
    ...
    def __build_mol_to_latent_model(self):
        ...
        if self.bn:
            states = BatchNormalization(
                momentum=self.bn_momentum, name="BN_2")(states)
    ...
    def __build_latent_to_states_model(self):
        ...
        for dec_layer in range(self.dec_layers):
            ...
            h_decoder = Dense(self.lstm_dim, activation="relu",
                              name=name)(latent_input)

            name = "Dense_c_" + str(dec_layer)
            c_decoder = Dense(self.lstm_dim, activation="relu",
                              name=name)(latent_input)
    ...
    def __build_batch_model(self):
        ...
        decoder_inputs = Input(
            shape=self.dec_input_shape, name="Decoder_Inputs")
        ...
        for dec_layer in range(self.dec_layers):
            ...
            if self.bn:
                x = BatchNormalization(
                    momentum=self.bn_momentum, name="BN_Decoder_" +
                    str(dec_layer)
                )(x)
        ...
        outputs = Dense(self.output_dims, activation="softmax",
                        name="Dense_Decoder")(x)
    ...
    def __build_model(self):
        ...
        if self.input_type == "mols":
            ...
            encoder_inputs = Input(
                shape=self.input_shape, name="Encoder_Inputs")
            ...
            self.__model = Model(
                inputs=[encoder_inputs, decoder_inputs], outputs=[x])

        # Input is pre-encoded, no need for encoder
        else:
            # Input tensor (MANDATORY)
            latent_input = Input(
                shape=(self.codelayer_dim,), name="Latent_Input")
            # Input tensor (MANDATORY) - Same as the batch_model input for teacher's forcing!
            decoder_inputs = Input(
                shape=self.dec_input_shape, name="Decoder_Inputs")
            ...
            self.__model = Model(
                inputs=[latent_input, decoder_inputs], outputs=[x])
    ...
    def __load(self, model_name):
        ...
        with tempfile.TemporaryDirectory() as dirpath:
            ...
            self.__build_sample_model(
                batch_input_length=1)  # Single-output model
            self.__build_sample_model(
                batch_input_length=256  # could also be self.batch_size
            )  # Multi-output model

        print("Loading finished in %i seconds." %
              ((datetime.now() - tstart).seconds))
    ...
    def fit(
        ...
        checkpoint_file = (
            checkpoint_dir +
            "%s--{epoch:02d}--{val_loss:.4f}--{lr:.7f}" % model_name
        )
        ...
        if gpus > 1:
            ...
            parallel_model.fit_generator(   # _generator
                self.train_gen,
            ...
            self.model.fit_generator(    # _generator
                self.train_gen,
                steps_per_epoch=self.steps_per_epoch / mini_epochs,
                epochs=mini_epochs * self.epochs,
                validation_data=self.valid_gen,
    ...
    def predict(self, latent, temp=1, rng_seed=None):
        ...
        for i in range(1000):
            ...
            if temp > 0:
                ...
                # print(f'nextCharProbs: {nextCharProbs}')
                nextCharProbs = (
                ...
                while IfMultinomial:
                    try:
                        ...
                        # print(e)
                        nextCharProbs = nextCharProbs - \
                            np.finfo(np.float32).epsneg
                        nextCharProbs = np.absolute(nextCharProbs)
                        continue
                # print(f'nextCharProbs: {nextCharProbs}')
    ...
    def predict_batch(self, latent, temp=1, rng_seed=None):
        ...
        if latent.shape[0] == 1:
            ...
            latent = np.ones(
                (self.batch_input_length, self.codelayer_dim)) * latent
        else:
        ...
        for i in range(1000):
            ...
            if temp > 0:
                ...
                nextCharProbs = (nextCharProbs.T /
                                 nextCharProbs.sum(axis=1) - 1e-8).T
                sampleidc = []
                for nextCharProb in nextCharProbs:
                    ...
                    while IfMultinomial:
                        try:
                            sampleidx = np.random.multinomial(
                                1, nextCharProb, 1).argmax()
                            IfMultinomial = False
                            sampleidc.append(sampleidx)
                            break
                        except Exception as e:
                            # print(e)
                            nextCharProb = (nextCharProb -
                                            np.finfo(np.float32).epsneg)
                            nextCharProb = np.absolute(nextCharProb)
                            continue
                sampleidc = np.asarray(sampleidc)

            else:
                sampleidc = np.argmax(o, axis=1)

            samplechars = [self.smilesvec1._int_to_char[idx]
                           for idx in sampleidc]
            ...
            for idx, samplechar in enumerate(samplechars):
                if not finished[idx]:
                    if samplechar != self.smilesvec1.endchar:
                        ...
                        samplevec = np.zeros(
                            (self.batch_input_length, 1,
                             self.smilesvec1.dims[-1])
                        )
    ...
    def get_graphs(self):
        ...
        try:
            ...
            if self.mol_to_latent_model is not None:
                plot_model(self.mol_to_latent_model,
                           to_file="mol_to_latent_model.png")
    ...
    def save(self, model_name):
        ...
        with tempfile.TemporaryDirectory() as dirpath:
            ...
            if self.mol_to_latent_model is not None:
                self.mol_to_latent_model.save(
                    dirpath + "/mol_to_latent_model.h5")

            self.latent_to_states_model.save(
                dirpath + "/latent_to_states_model.h5")
